...
pub inline fn object(comptime state: State, value: anytype) py.PyObject(state) {
    return tramp.Trampoline(state, @TypeOf(value)).asObject(value);
}
...
pub inline fn createOwned(comptime state: State, value: anytype) py.PyError!py.PyObject(state) {
    const trampoline = tramp.Trampoline(state, @TypeOf(value));
...
pub inline fn create(comptime state: State, value: anytype) py.PyError!py.PyObject(state) {
    return tramp.Trampoline(state, @TypeOf(value)).wrap(value);
}

/// Python -> Zig. Return a Zig object representing the Python object.
pub inline fn as(comptime state: State, comptime T: type, obj: anytype) py.PyError!T {
    return tramp.Trampoline(state, T).unwrap(object(state, obj));
}

/// Python -> Pydust. Perform a checked cast from a PyObject to a given PyDust class type.
pub inline fn checked(comptime state: State, comptime T: type, obj: py.PyObject) py.PyError!T {
...
pub inline fn unchecked(comptime state: State, comptime T: type, obj: py.PyObject) T {
...
test "create" {
    ...
    const str = try py.PyString(state).create("Hello");
    try testing.expectEqual(@as(isize, 1), py.refcnt(state, str));

    const some_tuple = try py.create(state, .{str});
    defer some_tuple.decref();
    try testing.expectEqual(@as(isize, 2), py.refcnt(state, str));
...
test "createOwned" {
    ...
    const some_tuple = try py.createOwned(state, .{str});
    defer some_tuple.decref();
    try testing.expectEqual(@as(isize, 1), py.refcnt(state, str));
}
