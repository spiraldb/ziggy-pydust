...
* `PyFoo(root).checked(py.PyObject(root))` - checks a `PyObject(root)` is indeed a `PyFoo` before wrapping it up as one.
* `PyFoo(root).unchecked(py.PyObject(root))` - wraps a `PyObject(root)` as a `PyFoo` without checking the type.
